...
public class GameOver_Text : MonoBehaviour
{
	...
	[SerializeField] private AnimationCurve m_move_y_curve;
	...
	IEnumerator Start()
    {
		...
		//操作有効化
		this.transform.parent.GetComponent<GameOver>().EnableInputAction();
	...
	public IEnumerator Move()
	{
		...
		while (true) {
			this.transform.position += new Vector3(0.0f, m_move_y_curve.Evaluate(time), 0.0f);
			time += Time.deltaTime;

			if (m_move_y_curve.keys[m_move_y_curve.keys.Length - 1].time <= time) {
				yield break;
			}
